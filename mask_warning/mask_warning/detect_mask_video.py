...
vs = cv2.VideoCapture('rtsp://192.168.1.45:554/cam/realmonitor?channel=1&subtype=0&unicast=true&proto=Onvif')
frame = vs.read()
h,w = frame.shape[:2]
...
while True:
	...
	# face mask or not
	(locs, preds) = detect_and_predict_mask(imutils.resize(frame,width=400), faceNet, maskNet)
	...
	for (box, pred) in zip(locs, preds):
		...
		(startX, startY, endX, endY) = (np.int32(startX*zoomRate), np.int32(startY*zoomRate), np.int32(endX*zoomRate), np.int32(endY*zoomRate))
		...
		colorDetec = detectColor(frame,np.int32(startX+(endX-startX)/2),np.int32(endY+(endY-startY)/2))
